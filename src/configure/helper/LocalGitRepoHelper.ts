...
export class LocalGitRepoHelper {
     ...
     */
    public async commitAndPushPipelineFile(pipelineYamlPath: string, message: string, repositoryDetails: GitRepositoryParameters): Promise<string> {
        await this.gitReference.add(pipelineYamlPath);
        await this.gitReference.commit(message, pipelineYamlPath);
